...
export default function NavItem({ label, to, subMenu, className }: NavItemProps) {
    ...
    >
      {hasSubMenu ? (
        ...
        >
          ...
          <ChevronDown
            size={16}
            className={`text-slate-800 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
          />
        </button>
      ) : (
        <NavLink
          to={to || '#'}
          className={({ isActive }) =>
            `flex items-center text-lg h-12 px-2 lg:px-4 py-2 ${isActive ? 'font-bold' : 'font-medium'}`
          }
        >
          {label}
        </NavLink>
